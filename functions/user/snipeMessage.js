...
module.exports = (client) => {
  ...
  client.snipeMessage = async (user, target, interaction, message) => {
    try {
      ...
      const snipe = await client.snipes.get(
        target ? guildId + "" + target.id : channelId
      );
      ...
      const embed = new EmbedBuilder()
        .setAuthor({
          name: target
            ? `${user.username} snipped message of ${
                target.username || target.user.username
              }`
            : "Last deleted message:",
          iconURL:
            "https://media.discordapp.net/attachments/976364997066231828/1012217326424293416/snipe.png",
        })
