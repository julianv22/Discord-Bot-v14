    ...
    try {
      ...
      if (!reload) {
        (async () => {
          try {
            ...
            console.error(
              chalk.red("Error while reloading application (/) command"),
              e
            );
          }
        })();
      ...
      async function LoadCommands(name, folderName, commandFolders) {
        ...
        for (const folder of commandFolders) {
          ...
          const commandFiles = readdirSync(`./${folderName}/${folder}`).filter(
            (f) => f.endsWith(".js")
          );
          table.addRow(
            `📂 ${folder.toUpperCase()} [${commandFiles.length}]`,
            "─",
            "────────────",
            "📂"
          );
          ...
          commandFiles.forEach((file) => {
            ...
            table.addRow(
              "",
              i++,
              command.data ? command.data.name : command.name,
              "✅\u200b"
            );
            count++;
          });
