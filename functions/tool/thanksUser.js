...
module.exports = client => {
  ...
  client.thanksUser = async (user, author, interaction, message) => {
    try {
      ...
      if (!user)
        return msg.reply({ embeds: [{ color: 16711680, description: `\\❌ | You have to mention someone!` }], ephemeral: true }).then(m => {
          if (msg == message)
            setTimeout(() => {
              m.delete();
            }, 10000);
      ...
      if (user.id === author.id)
        return msg.reply({ embeds: [{ color: 16711680, description: `\\❌ | You can not thank yourself 😅!` }], ephemeral: true }).then(m => {
          if (msg == message)
            setTimeout(() => {
              m.delete();
            }, 10000);
