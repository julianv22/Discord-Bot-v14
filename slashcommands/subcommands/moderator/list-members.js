  ...
  async execute(interaction, client) {
    ...
    if (!isMod)
      return interaction.reply({
        embeds: [{ color: 16711680, description: `\\❌ | You do not have \`${cfg.modRole}\` permissions to use this command!` }],
        ephemeral: true,
      });
    ...
    if (members) {
      ...
      if (isMention === true) embed.setDescription(`**${msg}**\n\n` + members.join(stInline)).setFooter({ text: `Tổng số: [${members.length}]` });
      else embed.setDescription(`**${msg}**\n\n` + userName.join(stInline)).setFooter({ text: `Tổng số: [${members.length}]` });

      await interaction.editReply({ embeds: [embed] });
    } else {
      await interaction.editReply({
        embeds: [{ color: 16711680, description: `\❌ | Can not find members or role is incorrect!` }],
        ephemeral: true,
      });
    }
  },
