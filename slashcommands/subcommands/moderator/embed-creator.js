...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    const titleInput = new ActionRowBuilder().addComponents(
      new TextInputBuilder().setCustomId('title').setLabel(`Embed's Title:`).setRequired(true).setStyle(TextInputStyle.Short)
    );

    const descriptionInput = new ActionRowBuilder().addComponents(
      new TextInputBuilder().setCustomId('description').setLabel(`Embed's Description:`).setRequired(true).setStyle(TextInputStyle.Paragraph)
    );

    const colorInput = new ActionRowBuilder().addComponents(
      new TextInputBuilder().setCustomId('color').setLabel(`Embed's Color`).setValue('Random').setRequired(true).setStyle(TextInputStyle.Short)
    );

    const thumbnailInput = new ActionRowBuilder().addComponents(
      new TextInputBuilder().setCustomId('thumbnailURL').setLabel('Thumbnail URL').setRequired(false).setStyle(TextInputStyle.Short)
    );
