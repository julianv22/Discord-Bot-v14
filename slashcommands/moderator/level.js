...
module.exports = {
  data: new SlashCommandBuilder()
    ...
    .addIntegerOption((opt) =>
      opt.setName("week").setDescription("Tuần").setRequired(true)
    )
    .addStringOption((opt) =>
      opt.setName("image").setDescription("Image URL").setRequired(true)
    ),
  category: "moderator",
  ...
  async execute(interaction, client) {
    ...
    if (week < 1)
      return interaction.reply({
        embeds: [
          { color: 16711680, description: `\\❌ | Số tuần phải lớn hơn 0` },
        ],
        ephemeral: true,
      });
