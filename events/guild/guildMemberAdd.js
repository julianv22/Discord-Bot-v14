...
module.exports = {
  ...
  try {
    ...
    const embWelcome = new EmbedBuilder()
      ...
      .addFields([{ name: `Bạn là thành viên thứ ${guild.memberCount} của server`, value: 'Chúc bạn một ngày làm việc vui vẻ!' }])
      .setColor('#00BCE3')
      .setThumbnail(user.displayAvatarURL(true))
      .setImage(cfg.welcomePNG)
      .setFooter({ text: guild.name, iconURL: guild.iconURL(true) })
      .setTimestamp();
    if (profile?.welomeMessage) embWelcome.addFields([{ name: `Server's Information:`, value: profile?.welomeMessage }]);
    ...
    channel.send({ embeds: [embWelcome] })
    // .then(() => {
    ...
    const emLog = new EmbedBuilder()
      ...
      .setTimestamp()
      .addFields({ name: 'Tên người dùng:', value: user.tag, inline: true }, { name: 'ID:', value: `||${user.id}||`, inline: true });
