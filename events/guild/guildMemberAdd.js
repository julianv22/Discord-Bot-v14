...
module.exports = {
  ...
  async execute(member, client) {
    try {
      ...
      ctx.fillStyle = "#FFF";
      addContext(
        `You are ${guild.memberCount}th member!`,
        c.w / 2,
        c.h - 140,
        32
      );
      ...
      const embWelcome = new EmbedBuilder()
        ...
        .setTitle("Welcome 👋")
        .setDescription(
          `Chào mừng ${user} tham gia server **${guild.name}!**  😍`
        )
        .addFields([
          {
      ...
      if (profile?.welomeMessage)
        embWelcome.addFields([
          { name: `Server's Information:`, value: profile?.welomeMessage },
        ]);
      ...
      console.error(
        chalk.yellow.bold("Error while running guildMemberAdd event"),
        e
      );
    }
  },
