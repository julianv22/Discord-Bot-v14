...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (args.join(' ').trim() === '?') return client.cmdGuide(message, this.name, this.description, this.aliases, prefix + this.name + ' <username>');

    if (!args[0])
      return channel.send({ embeds: [{ color: 16711680, description: `\\❌ | Hãy nhập username!` }] }).then(m => {
        setTimeout(() => {
          m.delete();
        }, 10000);
      });
    ...
    fetch(`https://api.github.com/users/${args.join('-')}`)
      ...
      .then(body => {
        if (body.message)
          return channel
            .send({ embeds: [{ color: 16711680, description: `\\❌ | Không tìm thấy người dùng, hãy nhập chính xác username!` }] })
            .then(m => {
              setTimeout(() => {
                m.delete();
              }, 10000);
